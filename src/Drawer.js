...
export default class Drawer extends Component {
  ...
  componentWillMount() {
    const { drawerWidth, drawerPosition, maskAlpha } = this.props;
    this.isLeft =
      drawerPosition === positions.Both || drawerPosition === positions.Left;
    this.isRight =
      drawerPosition === positions.Both || drawerPosition === positions.Right;
    ...
    this._pan = PanResponder.create({
      onStartShouldSetPanResponder: this._onStartShouldSetPanResponder.bind(
        this
      ),
      onStartShouldSetPanResponderCapture: this._onStartShouldCapture.bind(
        this
      ),
      onMoveShouldSetPanResponder: this._onMoveShouldSetPanResponder.bind(this),
      onMoveShouldSetPanResponderCapture: this._onMoveShouldCapture.bind(this),
      onPanResponderTerminationRequest: this._handleTerminationRequest.bind(
        this
      ),
  ...
  _onStartShouldSetPanResponder(evt, gestureState) {
    ...
    let singleDisabled =
      (this.isLeftOpen && leftDisabled) || (this.isRightOpen && rightDisabled);
    if (isOpen && !this.inAnimation && !disabled && !singleDisabled)
      return true;
    return false;
  }
  _onMoveShouldSetPanResponder(evt, gestureState) {
    ...
    if (
      this.props.disabled ||
      (this.props.responderNegotiate &&
        this.props.responderNegotiate(evt, gestureState) === false)
    )
      return false;
  ...
  _handlePanResponderMove(evt, gestureState) {
    ...
    if (dx > 0 && dx <= this.MAX_DX) {
      // swipe right
      if (this.isRight && this.isRightOpen)
        return this._updateNativeStyles(-this.MAX_DX + dx);
      if (this.isLeft && !this.isLeftOpen) this._updateNativeStyles(dx);
    } else if (dx < 0 && dx >= -this.MAX_DX) {
      // swipe left
      if (this.isLeft && this.isLeftOpen)
        return this._updateNativeStyles(this.MAX_DX + dx);
  ...
  _getCurrentDrawerWidth() {
    return this.isLeftActive
      ? this.styles.leftDrawer.style.left + this.MAX_DX
  ...
  closeDrawer() {
    ...
    const { duration, easingFunc = t => t } = this.props;
  ...
  _updateNativeStyles(dx) {
    ...
    this._rightDrawer &&
      this._rightDrawer.setNativeProps(this.styles.rightDrawer);
  ...
  render() {
    ...
    let drawerContent = this.props.drawerContent
      ? this._bindDrawerRef(this.props.drawerContent)
      : null;
    let leftDrawerContent = this.props.leftDrawerContent
      ? this._bindDrawerRef(this.props.leftDrawerContent)
      : null;
    let rightDrawerContent = this.props.rightDrawerContent
      ? this._bindDrawerRef(this.props.rightDrawerContent)
      : null;
    return (
      <View style={styles.container}>
        ...
        {this.isLeft && (
          <View
            ...
            style={[
              this.isLeft ? customStyles.drawer : {},
              customStyles.leftDrawer,
              styles.absolute
            ]}
          >
        ...
        {this.isRight && (
          <View
            ...
            style={[
              this.isRight ? customStyles.drawer : {},
              customStyles.rightDrawer,
              styles.absolute
            ]}
          >
